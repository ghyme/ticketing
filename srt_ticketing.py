...
def search_train(driver, dpt_stn, arr_stn, dpt_dt, dpt_tm, check_only = False, check_from=1, check_to=3, want_reserve=False):
    ...
    for i in range(check_from, check_to+1):
        text_arr_time = driver.find_element(By.CSS_SELECTOR, f"#result-form > fieldset > div.tbl_wrap.th_thead > table > tbody > tr:nth-child({i}) > td:nth-child(4)").text.replace("\n"," ")
        text_dpt_time = driver.find_element(By.CSS_SELECTOR, f"#result-form > fieldset > div.tbl_wrap.th_thead > table > tbody > tr:nth-child({i}) > td:nth-child(5)").text.replace("\n"," ")
...
if __name__ == "__main__":
    driver = open_brower()
    driver = login(driver, '2294550694', 'han2kdel#')
    search_train(driver, "수서", "대전", "20231019", "14", False, 1, 1, False) #기차 출발 시간은 반드시 짝수
